---
name: figma
description: Compare the current page with a Figma design
---
...
- `figma-url` (required): Figma file URL. Tool uses `figmaFileKey` and `figmaNodeId` (extract from URL: file key from path, node-id from query).
